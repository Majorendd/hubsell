...
local function Serverhop(NotPlaza)
    ...
    while task.wait() do
        ...
        if table.find(Servers, RandomServer.JobID) then continue end
        if #Servers >= 7 then
...
local function ValidateItem(BoothItem, WantedItem)
    if WantedItem.ID:find("All Huges") then
        if not BoothItem.IsHuge then return false end
    elseif WantedItem.ID:find("All Titanics") then
        if not BoothItem.IsTitanic then return false end
    elseif WantedItem.ID:find("All Exclusives") then
        if (not BoothItem.IsExclusive or BoothItem.IsHuge or BoothItem.IsTitanic) or BoothItem.Class ~= "Pet" then return false end
    end
    if WantedItem.ID:find("All Rarity") then
        if not BoothItem.Rarity or (BoothItem.Rarity:gsub(" ", "") ~= WantedItem.ID:split(":")[2]:gsub(" ", "") or BoothItem.IsHuge or BoothItem.IsTitanic) or BoothItem.Class ~= "Pet" then return false end
    elseif WantedItem.ID:find("All Class") then
        if not BoothItem.Class or (BoothItem.Class ~= WantedItem.ID:split(":")[2]:gsub(" ", "")) then return false end
    elseif WantedItem.ID:find("RAP Above") then
        if not BoothItem.RAP or (tonumber(BoothItem.RAP) < tonumber(RemoveSuffix(WantedItem.ID:split(":")[2]:gsub(" ", "")))) then return false end
    elseif WantedItem.ID:find("Difficulty Above") then
        if not BoothItem.Difficulty or (BoothItem.Difficulty and tonumber(BoothItem.Difficulty) < tonumber(RemoveSuffix(WantedItem.ID:split(":")[2]:gsub(" ", "")))) then return false end
    elseif WantedItem.ID:find("Name Find") then
        local Match = WantedItem.ID:split(": ")[2] 
        if not BoothItem.ID:find(Match) then return false end
    elseif WantedItem.ID ~= BoothItem.ID and not WantedItem.ID:find("All ") then
        return false
    end
    if WantedItem.Class ~= nil and WantedItem.Class ~= BoothItem.Class then return false end
    if not WantedItem.AllTypes then
        ...
        if (WantedItem.Rainbow and not BoothItem.Rainbow) or (BoothItem.Rainbow and not WantedItem.Rainbow) then return false end
        if (WantedItem.Golden and not BoothItem.Golden) or (BoothItem.Golden and not WantedItem.Golden) then return false end
    end
    if not WantedItem.AllTiers and (WantedItem.Tier and BoothItem.Tier) then
        if tonumber(WantedItem.Tier) ~= tonumber(BoothItem.Tier) then return false end
    end
    return true
end
...
local function DetermineTrend(Data)
    ...
    if math.abs(deviation) > MANIPULATION_THRESHOLD then return "Manipulated" end
    if math.abs(CurrentRAP - StartingRAP) <= 0.1 * StartingRAP then return "Stable"
    elseif CurrentRAP > StartingRAP then return "Increasing"
    else return "Decreasing" end
end
...
local function FindItem(Data, ReturnAmount)
    ...
    for _, Inventory in pairs(Inventories) do
        ...
        for UID, ItemTable in pairs(Inventory._byUID) do
            ...
            if ItemInfo.Shiny then ItemInfo.Display = "Shiny" end
...
local function GetMailCost()
    ...
    if PlayerSave.Get().Gamepasses.VIP or LocalPlayer:GetAttribute("Partner") then return BaseCost end
    return Cost
end
...
local function SellerNotification(CurrentInfo)
    ...
    local Description = {
        ...
        "**<:Booth:1239350605294604378> Booth Count:** `"..(ItemCount and AddCommas(tostring(ItemCount)) or "?").."`",
...
if Settings.Seller and Settings.Seller.Active and FileSettings.Seller then
    local function IsBoothAvailable(BoothID)
        for _, BoothTable in pairs(ClaimedBooths) do
            if BoothTable.BoothID == BoothID then return false end
        end
        return true
    end
    ...
    local function ClaimOptimalBooth()
        ...
        for BoothID, BoothModel in next, BoothsInteractive do
            ...
            if IsBoothAvailable(BoothID) then
                ...
                table.insert(BoothCandidates, {BoothID = BoothID, Model = BoothModel, Y = yPriority, xDistance = xDistance})
            end
        end
        table.sort(BoothCandidates, function(a, b)
            if a.Y == b.Y then return a.xDistance < b.xDistance end
            return a.Y < b.Y
        end)
        ...
        for Class, ClassTable in next, Info["Given"] do
            for UID, Items in ClassTable do
                ...
                if UI["URL"] and not table.find(LastUIDDs, UID) then
                    ...
                    if ItemData then
                        task.wait(1)
                        SellerNotification({Amount = Items._am or 1, Spent = ItemCost, ID = Items.id, Icon = ItemData.Icon, Name = ItemData.Display, Class = Class})
                    end
                end
            end
        end
    ...
    local function ProcessItem(Name, Data)
        ...
        repeat task.wait()
            ...
            if PriceData.MinPrice and PriceData.RealPrice < PriceData.MinPrice then PriceData.RealPrice = PriceData.MinPrice end
            if PriceData.MaxPrice and PriceData.RealPrice > PriceData.MaxPrice then PriceData.RealPrice = PriceData.MaxPrice end
