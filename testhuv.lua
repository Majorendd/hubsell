...
if not Rayfield then
    warn(\"[Plaza Plus]: Failed to load Rayfield. Please make sure you have internet access.\")
    return
end

local function CreateStatusLabel()
    local gui = Instance.new(\"ScreenGui\")
    gui.Name = \"PlazaPlusStatusGUI\"
    ...
    label.Text = \"Configure items, then press START\"
    label.Parent = frame
    return label
end

local botStatus = CreateStatusLabel()
local function UpdateBotStatus(text, color)
    botStatus.Text = text or botStatus.Text
    if color then botStatus.TextColor3 = color end
end

local Window = Rayfield:CreateWindow({
    Name = \"PLAZA PLUS\",
    LoadingTitle = \"Plaza Plus\",
    LoadingSubtitle = \"Seller + Sniper\",
    ConfigurationSaving = {Enabled = false},
    Discord = {Enabled = false},
    KeySystem = false,
    IntroText = \"Seller + Sniper dashboard\",
})

local SellerTab = Window:CreateTab(\"Seller\")
...
local function PersistSellerItems()
    Cfg.SellerItems = {}
    for _, v in pairs(SellerItemData) do
        table.insert(Cfg.SellerItems, v)
    end
    SaveCfg()
end
...
local function DoAddItem(name, price, amt, cls, tier, prio, rainbow, golden, shiny, allTypes)
    ...
    price = tostring(price or \"\"):match(\"^%s*(.-)%s*$\")
    if name == \"\" or price == \"\" then
        UpdateBotStatus(\"⚠ Name and price required\", C.Red)
        task.delay(2, function() UpdateBotStatus(\"Configure items, then press START\", C.Sub) end)
        return
    end
    ...
    if SellerItemData[displayKey] then
        UpdateBotStatus(\"⚠ Already in list\", C.Red)
        task.delay(2, function() UpdateBotStatus(\"Configure items, then press START\", C.Sub) end)
        return
    end
    ...
    PersistSellerItems()
    if IsRunning then
        UpdateBotStatus(\"⚡ Added live — listing next cycle\", C.Yellow)
    else
        UpdateBotStatus(\"✓ Added: \"..displayKey, C.Green)
    end
end

local sellerSection = SellerTab:CreateSection(\"Add Seller Item\")
sellerSection:CreateTextBox({
    Name = \"Item Name\",
    Placeholder = \"e.g. Spring Bluebell Token\",
    RemoveTextAfterFocusLost = false,
    Callback = function(value) sellerState.Name = value end,
})
sellerSection:CreateTextBox({
    Name = \"Price\",
    Placeholder = \"e.g. 2 or 1m or +20%\",
    RemoveTextAfterFocusLost = false,
    Callback = function(value) sellerState.Price = value end,
})
sellerSection:CreateTextBox({
    Name = \"Amount\",
    Placeholder = \"blank = max available\",
    RemoveTextAfterFocusLost = false,
    Callback = function(value) sellerState.Amount = value end,
})
sellerSection:CreateTextBox({
    Name = \"Class\",
    Placeholder = \"e.g. Misc\",
    RemoveTextAfterFocusLost = false,
    Callback = function(value) sellerState.Class = value end,
})
sellerSection:CreateTextBox({
    Name = \"Tier\",
    Placeholder = \"e.g. 2\",
    RemoveTextAfterFocusLost = false,
    Callback = function(value) sellerState.Tier = tonumber(value) end,
})
sellerSection:CreateToggle({
    Name = \"Priority (list first)\",
    CurrentValue = false,
    Callback = function(value) sellerState.Priority = value end,
})
sellerSection:CreateToggle({
    Name = \"Rainbow\",
    CurrentValue = false,
    Callback = function(value) sellerState.Rainbow = value end,
})
sellerSection:CreateToggle({
    Name = \"Golden\",
    CurrentValue = false,
    Callback = function(value) sellerState.Golden = value end,
})
sellerSection:CreateToggle({
    Name = \"Shiny\",
    CurrentValue = false,
    Callback = function(value) sellerState.Shiny = value end,
})
sellerSection:CreateToggle({
    Name = \"All Types\",
    CurrentValue = false,
    Callback = function(value) sellerState.AllTypes = value end,
})
sellerSection:CreateButton({
    Name = \"＋ Add Item\",
    Callback = function()
        DoAddItem(
            sellerState.Name,
            sellerState.Price,
            sellerState.Amount ~= \"\" and tonumber(sellerState.Amount) or nil,
            sellerState.Class ~= \"\" and sellerState.Class or nil,
            sellerState.Tier,
            sellerState.Priority,
            sellerState.Rainbow,
            sellerState.Golden,
            sellerState.Shiny,
            sellerState.AllTypes
        )
    end,
})
sellerSection:CreateButton({
    Name = \"⚡ Quick: All Huges +20%\",
    Callback = function()
        DoAddItem(\"All Huges\", \"+20%\", nil, nil, nil, false, false, false, false, true)
    end,
})

local actionsSection = SellerTab:CreateSection(\"Actions\")
actionsSection:CreateButton({
    ...
    Callback = function()
        ...
        if (mode == \"Seller\" or mode == \"Both\") and #sellerItems == 0 then
            UpdateBotStatus(\"⚠ Add items to the Seller tab first!\", C.Red)
            task.delay(3, function() UpdateBotStatus(\"Configure items, then press START\", C.Sub) end)
            return
        end
        if (mode == \"Sniper\" or mode == \"Both\") and #sniperItems == 0 then
            UpdateBotStatus(\"⚠ Add targets to the Sniper tab first!\", C.Red)
            task.delay(3, function() UpdateBotStatus(\"Configure items, then press START\", C.Sub) end)
            return
        end
        IsRunning = true
        UpdateBotStatus(\"⏳ Starting \"..mode..\" mode...\", C.Green)
        if mode == \"Seller\" or mode == \"Both\" then task.spawn(RunSeller) end
        if mode == \"Sniper\" or mode == \"Both\" then task.spawn(RunSniper) end
...
actionsSection:CreateButton({
    ...
    Callback = function()
        ...
        UpdateBotStatus(\"Stopped. Reconfigure and press START again.\", C.Sub)
...
SniperTab:CreateSection(\"Coming Soon\"):CreateParagraph({
    Title = \"Sniper Status\",
    Content = \"Sniper is currently under development and will be available in a future update.\nSaved targets:\nBooth: \"..CountTable(Cfg.SniperItems)..\"  Terminal: \"..CountTable(Cfg.TerminalItems),
})

local settingsSection = SettingsTab:CreateSection(\"General\")
settingsSection:CreateDropdown({
    Name = \"Mode\",
    Options = {\"Seller\", \"Sniper\", \"Both\"},
    CurrentOption = Cfg.Mode or \"Seller\",
    Callback = function(option) Cfg.Mode = option; SaveCfg() end,
})
settingsSection:CreateToggle({
    Name = \"Switch Servers\",
    CurrentValue = Cfg.SwitchServers or false,
    Callback = function(value) Cfg.SwitchServers = value; SaveCfg() end,
})
settingsSection:CreateTextBox({
    Name = \"Switch Delay (minutes)\",
    Placeholder = tostring(Cfg.Delay or 20),
    RemoveTextAfterFocusLost = false,
    Callback = function(value) Cfg.Delay = tonumber(value) or Cfg.Delay; SaveCfg() end,
})
settingsSection:CreateToggle({
    Name = \"Only Pro Servers\",
    CurrentValue = Cfg.OnlyPro or false,
    Callback = function(value) Cfg.OnlyPro = value; SaveCfg() end,
})
...
webhookSection:CreateTextBox({
    ...
    RemoveTextAfterFocusLost = false,
    Callback = function(value) Cfg.WebhookURL = value; SaveCfg() end,
})
SettingsTab:CreateSection(\"Info\"):CreateParagraph({
    Title = \"Status\",
    Content = \"Use the Seller tab to add sell items. The Sniper tab shows coming soon info. Settings are saved automatically.\",
})

if not table.find({PS99.Normal,PS99.Pro,PETSGO.Normal,PETSGO.Pro},game.PlaceId) then
    UpdateBotStatus(\"⚠ Wrong game! Go to PS99 or PETS GO Trading Plaza.\", C.Red)
end

print(\"[Plaza Plus GUI]: Rayfield interface ready.\")-- ══════════════════════════════════════════
--  SELLER RUNTIME
-- ══════════════════════════════════════════
